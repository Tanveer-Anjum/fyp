...
//       try {
//         // Fetch all products (or you can filter if you want featured ones)
...
export default function FeaturedProducts({ categoryType = "new", limit = 8 }) {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchFeaturedProducts = async () => {
      try {
        const { data } = await axios.get(`http://localhost:8080/api/products?categoryType=${categoryType}`);
        setProducts(data.products.slice(0, limit)); // Show only 'limit' products
      } catch (err) {
  ...
  return (
    <section className="py-12 px-6 bg-gray-50">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">
        Featured {categoryType.charAt(0).toUpperCase() + categoryType.slice(1)} Products
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {loading
