...
const Chatbox = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const aiMsg = {
        role: "assistant",
        text: res.data.text,
        products: res.data.structuredProducts || [],
      };

      setMessages((prev) => [...prev, aiMsg]);

      if (res.data.newHistory) {
        setHistory(res.data.newHistory);
      }
    } catch (err) {
      console.error(err);
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: "AI Error. Please try again." },
      ]);
    }
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-10 p-4 shadow-lg rounded-xl bg-white">
      <h2 className="text-2xl font-bold mb-4">Bazario AI Assistant</h2>

      <div className="h-96 overflow-y-auto border p-3 rounded-lg">
        {messages.map((msg, index) => (
          <div key={index} className="mb-4">
            <p
              className={`p-2 rounded-lg text-white w-fit ${
                msg.role === "user" ? "bg-blue-600 ml-auto" : "bg-green-600"
              }`}
            >
              {msg.text}
            </p>
